...
const MessengingButton = ({target, children}) => {
    const navigate = useRouter();
    function handleClick(e) {
        e.preventDefault();
        navigate.push('/contact/'+target);
    }
    return (
        <button data-icon="message-circle" target="chat" onClick={ handleClick}>
            {children}
		</button>
	);
}
		...
		features: {
			description: [
				...
				<div key="contact" className="w-full flex justify-end gap-4">
					<MessengingButton target="chat">
						<MessageCircle color={'var(--background'} />
					</MessengingButton>
					<MessengingButton target="mail">
